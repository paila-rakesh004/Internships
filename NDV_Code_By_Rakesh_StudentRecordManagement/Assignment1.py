...
FILE_NAME = 'students.json'  #creating the file manually

def load_students():
    if not os.path.exists(FILE_NAME):
        return[]
    with open(FILE_NAME,'r') as file:  #open the file in the read format
        return json.load(file)
def save_students(students):
    with open(FILE_NAME,'w') as file:   #open the file in write format
        return json.dump(students,file)
def add_students():                    #function for adding the students
...
def display_students() :        #function for displaying the students
    students = load_students()
    if not students:
        print("No data Found")
    for det in students:
        ...
        print('-'*20)
def update_students():          #function for updating the students details
...
def delete_students():          #function for deleting the students from the json file
